...
export function Carousel({
  ...
  const maxOffset = useCallback(
    (node: HTMLDivElement) => Math.max(0, scrollSize(node) - clientSize(node)),
  ...
  const springBack = useCallback(
    (initialVelocity = 0) => {
      ...
      const tick = (time: number) => {
        ...
        previous = time;
        velocity +=
          (-physics.bounceTension * position - physics.bounceFriction * velocity) * seconds;
  ...
  const momentum = useCallback(
    (node: HTMLDivElement, initialVelocity: number) => {
  ...
  const record = (value: number) => {
    ...
    samplesRef.current = [...samplesRef.current, { value, time }].filter(
      (sample) => time - sample.time <= physics.sampleWindow,
    );
  };
  ...
  const onPointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    sessionRef.current = {
      ...
      startPrimary: primary(event),
      startCross: cross(event),
      startOffset: offset(node),
      captured: false,
      dragged: false,
    };
  };
  ...
  const onPointerMove = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    setRubberBand(
      desired < 0
        ? -desired * physics.overdragScale
        : desired > maximum
          ? -(desired - maximum) * physics.overdragScale
          : 0,
    );
    updateThumb(true);
  };
  ...
  const finish = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const velocity =
      first && last
        ? -((last.value - first.value) / Math.max(1, last.time - first.time)) *
