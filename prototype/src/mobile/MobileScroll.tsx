...
function shouldIgnoreScrollDrag(target: EventTarget | null) {
  return target instanceof Element && Boolean(target.closest('[data-scroll-drag="ignore"]'));
}
...
export function MobileScroll({ className, children }: MobileScrollProps) {
  ...
  const setRubberBand = useCallback((value: number) => {
    const clamped = Math.max(
      -scrollPhysics.maxOverdrag,
      Math.min(scrollPhysics.maxOverdrag, value),
    );
    ...
    (initialVelocity = 0) => {
      ...
      const step = (time: number) => {
        const deltaTime = Math.min(
          0.034,
          ((lastTime === null ? time : time - lastTime) || 16) / 1000,
        );
        lastTime = time;
        const acceleration =
          -scrollPhysics.bounceTension * position - scrollPhysics.bounceFriction * velocity;
        velocity += acceleration * deltaTime;
        position += velocity * deltaTime;

        if (
          Math.abs(position) < scrollPhysics.bounceTolerance &&
          Math.abs(velocity) < scrollPhysics.bounceTolerance
        ) {
          setRubberBand(0);
          inertiaFrameRef.current = null;
          return;
  ...
  const applyDragPosition = useCallback(
    (startScrollTop: number, movementY: number, scroll: HTMLDivElement) => {
  ...
  const startMomentum = useCallback(
    (scroll: HTMLDivElement, initialVelocity: number) => {
      ...
      const step = (time: number) => {
        const deltaTime =
          lastInertiaTimeRef.current === null
            ? 16
            : Math.min(34, time - lastInertiaTimeRef.current);
