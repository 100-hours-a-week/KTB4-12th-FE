...
test('mobile viewport fills the browser without preview device chrome', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/tests/runtime-fixture.html');

  await expect(page.getByTestId('device-menu-bar')).toBeHidden();
  await expect(page.locator('.phone-bezel')).toBeHidden();
  await expect(page.locator('.mobile-cursor')).toBeHidden();
  await expect(page.getByLabel('Device status bar')).toBeHidden();
  await expect(page.getByTestId('home-indicator')).toBeHidden();
  await expect(page.getByTestId('mobile-app-viewport')).toHaveCSS('bottom', '0px');

  const layout = await page.evaluate(() => {
    const screen = document.querySelector<HTMLElement>('[data-testid="device-screen"]')!;
    const frame = document.querySelector<HTMLElement>('[data-testid="phone-frame"]')!;
...
test('mobile Android hides its simulated system navigation and uses the full height', async ({
  page,
}) => {
  ...
  await expect(page.getByLabel('Device status bar')).toBeHidden();
  await expect(page.getByTestId('android-navigation-bar')).toBeHidden();
  await expect(page.getByTestId('mobile-app-viewport')).toHaveCSS('bottom', '0px');
});
...
test('vertical intent over a carousel is handed to MobileScroll in both directions', async ({
  page,
}) => {
  const card = page.locator('.carousel-card').nth(1);
  const carousel = page.locator('.fixture-carousel');
...
test('switching to Pixel keeps the composer above Android navigation', async ({ page }) => {
  ...
  await expect(page.locator('.phone-bezel')).toHaveAttribute('src', '/assets/android/Pixel10.png');
  await expect(statusIndicators).toHaveAttribute('data-platform', 'android');
  await expect(statusIndicators).toHaveAttribute('src', '/assets/status/status-icons.svg');
  ...
  const layout = await page.evaluate(() => {
    ...
    return {
      ...
      safeAreaBottom: Number.parseFloat(
        getComputedStyle(
          document.querySelector<HTMLElement>('[data-testid="device-screen"]')!,
        ).getPropertyValue('--device-safe-area-bottom'),
      ),
    };
  });
  ...
  expect(Math.abs(keyboardLayout.keyboardBottom - keyboardLayout.screenBottom)).toBeLessThanOrEqual(
    1,
  );
